...
INTERVALO_ACTUALIZACION_ESTADO_SEGUNDOS = 1.0
...
def crear_interfaz() -> gr.Blocks:
    ...
    with gr.Blocks(title=TITULO_PROYECTO, css=CSS_INTERFAZ) as interfaz:
        ...
        def actualizar_estado_si_cambio():
            ...
            if estado_actual == ultimo_estado_mostrado["valor"]:
                return gr.skip()

            ultimo_estado_mostrado["valor"] = estado_actual
            return armar_html_estado(estado_actual)
        ...
        camara = WebRTC(
            ...
            full_screen=False,
            mirror_webcam=False,
            rtc_configuration=obtener_configuracion_turn if usar_turn else None,
        ...
        temporizador_estado.tick(
            fn=actualizar_estado_si_cambio,
...
if __name__ == "__main__":
    print("Creando interfaz Gradio...", flush=True)
    app = crear_interfaz()
    app.queue(default_concurrency_limit=1)
    print("Iniciando servidor en 0.0.0.0:7860...", flush=True)
